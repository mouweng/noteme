...
- 字符串索引和切片(索引返回值是byte，也就是uint8类型)

```go
s := "hello"
s[0] // 输出 104（字符 'h' 的 ASCII 值），数据结构是byte（uint8 类型）
s[1:4] // 输出 "ell"（截取从索引 1 到索引 4 的子字符串，不包含 4）
```
...
```go
// 者使用 strings.Builder，在大量拼接操作时性能更高：
import "strings"

var builder strings.Builder
builder.WriteString("leet")
builder.WriteString("code")
fmt.Println(builder.String()) // 输出 "leetcode"
```
...
```go
value := m2[i] // 如果不存在，则value为值类型的默认值
value, exist := mp[i]
```
